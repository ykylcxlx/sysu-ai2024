...
class ChessBoard(object):
    ...
    def __init__(self, screen, smallest=False):
        """初始化"""
        if smallest:
            self.smallest = True
            return
        
        self.last12hash = [i for i in range(12)]
        self.smallest = False
    ...
    def map_hash(self) -> int:
        joined_str = ''
        for row in range(10):
            for col in range(9):
                if self.chessboard_map[row][col]:
                    joined_str += self.chessboard_map[row][col].team + '_' + self.chessboard_map[row][col].name + '_'
                else:
                    joined_str += 'None_'
        return hash(joined_str)
        
    
    def copy_smallest(self) -> 'ChessBoard':
        new_chessboard = ChessBoard(None, smallest=True)
        new_chessboard.chessboard_map = [[None for _ in range(9)] for _ in range(10)]
        for row, line in enumerate(new_chessboard.chessboard_map):
            for col, _ in enumerate(line):
                if self.chessboard_map[row][col] is not None:
                    new_chessboard.chessboard_map[row][col] = self.chessboard_map[row][col].copy_smallest()
                else:
                    new_chessboard.chessboard_map[row][col] = None
        new_chessboard.capture_count = self.capture_count
        new_chessboard.last12hash = copy.deepcopy(self.last12hash)
        new_chessboard.winner = self.winner
    ...
    def get_put_down_position(self, clicked_chess, delete_general=True, none_when_end=False):
        """获取当前被点击棋子可以落子的位置坐标"""
        if none_when_end:
            if self.winner:
                return []
    ...
    def move_chess(self, new_row, new_col):  # ToDo: 新增判断是否吃了对方棋子
        ...
        # 清楚之前位置为None
        self.chessboard_map[old_row][old_col] = None
        self.last12hash.pop(0)
        self.last12hash.append(self.map_hash())
        
        
    def move_chess2(self, old_row, old_col, new_row, new_col):  # ToDo: 新增判断是否吃了对方棋子
        """
        将棋子移动到指定位置
        """
        ...
        if self.chessboard_map[new_row][new_col] != None:
            ...
            self.capture_count += 1
        move_team = self.chessboard_map[old_row][old_col].team
        # 移动位置
        self.chessboard_map[new_row][new_col] = self.chessboard_map[old_row][old_col]
        # 修改棋子的属性
        self.chessboard_map[new_row][new_col].update_position(new_row, new_col)
        ...
        self.last12hash.pop(0)
        self.last12hash.append(self.map_hash())
        if self.judge_repeat() and self.winner is None:
            self.winner = 'r' if move_team == 'b' else 'b'
        if self.judge_draw():
            self.winner = 'd'
        if self.judge_win("r"):
            self.winner = "r"
    ...
    def judge_repeat(self) -> bool:
        # 如果重复走子3次以上
        same_count = 0
        for i in range(12):
            if self.last12hash[i] == self.last12hash[-1]:
                same_count += 1
        if same_count >= 3:
            return True
        else:
