...
def main():
    ...
    # 创建AI对象
    # ai = ChessAI(game.computer_team if my_team == "r" else game.user_team) 
    # my_ai = MyAI(my_team)

    #这里两次都是我方是黑棋，一次先手一次后手
    ai = ChessAI(game.user_team) 
    my_ai = MyAI(game.computer_team)

    print("Total games 1/2 ")
    # 主循环s
    while True:
        # 显示游戏背景
        ...
        if not game.show_win and not game.show_draw and game.AI_mode and game.get_player() == ai.team:
            if game.back_button.is_repeated():
                ...
                print("ai_next_step",cur_row, cur_col, nxt_row, nxt_col)
                # # 选择棋子
                ...
                game.exchange()
        #我的ai行动
        if not game.show_win and not game.show_draw and game.AI_mode and game.get_player() == my_ai.team:
            if game.back_button.is_repeated():
                print("获胜...")
                game.set_win(game.get_player()) 
            else:
                ...
                print("my_ai_next_step",cur_row, cur_col, nxt_row, nxt_col)
                 # 选择棋子
                ClickBox(screen, cur_row, cur_col)
                #  # 下棋子
                chessboard.move_chess(nxt_row, nxt_col)
